"""QualiteAirSource — orchestrateur download / preprocess / process / load."""
...
from src.sources.qualite_air import config, download, load, preprocess, process

logger = logging.getLogger(__name__)


class QualiteAirSource(DataSource):
    def __init__(
        self,
    ...
    def download(self) -> None:
        logger.info("=== [QualiteAir] Download ===")
        download.run(self.raw_dir)

    def preprocess(self) -> None:
        logger.info("=== [QualiteAir] Preprocess ===")
        preprocess.run(self.raw_dir)

    def process(self) -> None:
        logger.info("=== [QualiteAir] Process ===")
        process.run(self.raw_dir, self.processed_dir)

    def load(self) -> None:
        logger.info("=== [QualiteAir] Load (PostgreSQL) ===")
