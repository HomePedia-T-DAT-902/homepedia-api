"""CrimeSource — orchestrateur download / preprocess / process / load."""
...
from src.sources.crime import config
from src.sources.crime import download, load, preprocess, process

logger = logging.getLogger(__name__)


class CrimeSource(DataSource):
    def __init__(
        self,
    ...
    def download(self) -> None:
        logger.info("=== [Crime] Download ===")
        download.run(self.raw_dir)

    def preprocess(self) -> None:
        logger.info("=== [Crime] Preprocess ===")
        preprocess.run(self.raw_dir)

    def process(self) -> None:
        logger.info("=== [Crime] Process (Spark) ===")
        process.run(self.raw_dir, self.processed_dir)

    def load(self) -> None:
        logger.info("=== [Crime] Load (PostgreSQL) ===")
