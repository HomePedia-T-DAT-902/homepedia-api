...
Usage:
    python -m src.sources.ville_ideale.preprocess
"""
...
def run(raw_dir: Path = config.RAW_DIR) -> None:
    cities = read_jsonl(raw_dir / config.CITIES_FILE)
    reviews = read_jsonl(raw_dir / config.REVIEWS_FILE)
    if not cities:
        logger.warning("No raw cities found in %s — nothing to preprocess", raw_dir)
        return
    records = build_records(cities, reviews)
    out = raw_dir / config.COMMUNES_FILE
    write_jsonl(out, records)
    logger.info("Preprocessed %d communes (%d reviews) -> %s", len(records), len(reviews), out)
