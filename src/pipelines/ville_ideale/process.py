...
from src.pipelines.ville_ideale.io_utils import ENRICHED, PREPROCESSED, read_jsonl, write_jsonl
...
def run() -> None:
    records = read_jsonl(PREPROCESSED)
    if not records:
        logger.warning("No preprocessed records in %s — run preprocess first", PREPROCESSED)
        return
    enrich(records)
    write_jsonl(ENRICHED, records)
    logger.info("Enriched %d communes with word clouds -> %s", len(records), ENRICHED)
