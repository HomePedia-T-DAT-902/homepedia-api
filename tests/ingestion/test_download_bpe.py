...
from src.ingestion.download_bpe import (
    decompress_gz,
    download_bpe,
    download_file,
...
def test_get_bpe_resource_url_empty_raises():
    mock_response = MagicMock()
    mock_response.raise_for_status = MagicMock()
...
def test_get_bpe_resource_url_prefers_gz_over_plain():
    """Un fichier .csv.gz doit être préféré si le titre contient 'ensemble'."""
    mock_response = MagicMock()
    mock_response.raise_for_status = MagicMock()
    mock_response.json.return_value = {
        "resources": [
            {"title": "BPE ensemble 2024", "format": "csv+gz", "url": "http://ex.com/bpe_ensemble.csv.gz"},
            {"title": "BPE ensemble 2024 (CSV)", "format": "csv", "url": "http://ex.com/bpe_ensemble.csv"},
        ]
    }

    with patch("src.ingestion.download_bpe.requests.get", return_value=mock_response):
        url, _ = get_bpe_resource_url()
...
def test_download_bpe_skips_existing(tmp_path):
    ...
    with patch("src.ingestion.download_bpe.RAW_DIR", tmp_path):
        with patch("src.ingestion.download_bpe.get_bpe_resource_url") as mock_api:
            download_bpe(force=False)
...
def test_download_bpe_force_redownloads(tmp_path):
    ...
    with patch("src.ingestion.download_bpe.RAW_DIR", tmp_path):
        with patch("src.ingestion.download_bpe.get_bpe_resource_url", return_value=("http://ex.com/bpe.csv", "BPE")):
            with patch("src.ingestion.download_bpe.download_file") as mock_dl:
                download_bpe(force=True)
                mock_dl.assert_called_once()


def test_download_bpe_decompresses_gz(tmp_path):
    """Si l'URL pointe vers un .gz, le fichier doit être décompressé."""
    # Créer un bpe.csv factice pour que _log_summary ne plante pas
    (tmp_path / "bpe.csv").write_text("AN;DEPCOM;TYPEQU\n2024;75056;A101\n")

    with patch("src.ingestion.download_bpe.RAW_DIR", tmp_path):
        with patch(
            "src.ingestion.download_bpe.get_bpe_resource_url",
            return_value=("http://ex.com/bpe.csv.gz", "BPE"),
        ):
            with patch("src.ingestion.download_bpe.download_file"):
                with patch("src.ingestion.download_bpe.decompress_gz") as mock_decomp:
                    download_bpe(force=True)
